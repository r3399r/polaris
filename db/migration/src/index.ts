...
import {
  CockroachDataSource,
  LogApiEntity as CockroachLogApiEntity,
} from './dataAccess/CockroachDataSource';
import {
  MySqlDataSource,
  LogApiEntity as MySqlLogApiEntity,
} from './dataAccess/MySqlDataSource';
...
async function main() {
  ...
  try {
    while (hasMore) {
      ...
      const logs = await CockroachDataSource.getRepository(
        CockroachLogApiEntity
      )
